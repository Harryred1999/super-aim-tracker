...
def send_discord_embed(ticker, signal_type, current_price, true_break_even, stop_loss, trailing_stop_target, rr_ratio, volume_ratio, turnover, recommended_shares, rsi_val, color):
    ...
    embed = {
        ...
        "footer": {"text": f"AIM Engine V20 • Pro-Terminal Algorithmic Edition"},
...
def send_summary_digest(scanned_count, buoys_found, watch_found, regime_status):
    ...
    embed = {
        "title": f"📊 End-of-Day Pro-Terminal Summary Digest",
        "color": 3447003,
        "description": f"Quantitative audit completed for today's session.",
        "fields": [
            {"name": "🔍 Total Scanned", "value": f"`{scanned_count}`", "inline": True},
            {"name": "🌐 Market Regime", "value": f"`{status_text}`", "inline": True},
            {"name": "🚨 Strong Buys", "value": f"`{buoys_found}`", "inline": True},
            {"name": "⭐ Watchlist Setups", "value": f"`{watch_found}`", "inline": True}
        ],
        "footer": {"text": f"AIM Engine V20 • Automated Audit Complete"},
        "timestamp": pd.Timestamp.utcnow().isoformat()
    }
    requests.post(WEBHOOK_URL, json={"embeds": [embed]})
...
def analyze_stock(ticker):
    try:
        ...
        if is_golden_cross and is_above_trend and weekly_trend_ok and rsi_healthy and volume_ratio >= MIN_VOLUME_MULTIPLIER and rr_ratio >= MIN_RISK_REWARD_RATIO:
            return ("BUY", current_price, true_break_even_price, stop_loss, trailing_stop_target, rr_ratio, volume_ratio, daily_turnover, recommended_shares, current_rsi)
            ...
            if distance_to_ma <= 0.008 and weekly_trend_ok and rsi_healthy and rr_ratio >= MIN_RISK_REWARD_RATIO:
                return ("WATCH", current_price, true_break_even_price, stop_loss, trailing_stop_target, rr_ratio, volume_ratio, daily_turnover, recommended_shares, current_rsi)
...
if __name__ == "__main__":
    ...
    print(f"Executing V20 Pro-Terminal audit in [{SCAN_MODE}] mode across {len(tickers)} symbols...")
    ...
    print(f"Pro-Terminal Audit Complete for Mode: {SCAN_MODE}")
